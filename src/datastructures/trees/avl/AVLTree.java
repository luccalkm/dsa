...
import datastructures.arraylist.DynamicArray;
...
public class AVLTree<T extends Comparable<T>> {
	...
	private Node<T> delete(Node<T> node, T val) {
		...
		if (val.compareTo(node.val) < 0)
			...
			/*
			 * Casos: 1. Não tem filhos 2. Tem filho na esquerda 3. Tem filho na direita 4.
			 * Tem dois filhos
			 */
	...
	public DynamicArray<T> toList() {
		DynamicArray<T> arr = new DynamicArray<T>();
		inOrder(root, arr);
		return arr;
	}

	private void inOrder(Node<T> node, DynamicArray<T> list) {
		if (node != null) {
